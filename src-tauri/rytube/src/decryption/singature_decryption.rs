//Thanks to https://github.com/DzenanJupic/rustube/blob/master/src/descrambler/cipher.rs
...
use crate::{Error, TryCollect};

pub(crate) type TransformerFn = (fn(&mut Vec<u8>, Option<isize>), &'static str);

lazy_static!{
    static ref JS_FUNCTION_REGEX: Regex = Regex::new(r"\w+\.(\w+)\(\w,(\d+)\)").unwrap();
}
...
impl Cipher {
    pub(crate) fn from_js(js: &str) -> Result<Self, Error> {
        let transform_plan = get_transform_plan(js)?;

        let (var, _): (&str, &str) = transform_plan
            .get(0)
            .ok_or_else(|| Error::SignatureDecryptionError(
    ...
    pub(crate) fn decrypt_signature(&self, signature: String) -> Result<String, Error> {
        ...
        for js_fun_name in self.transform_plan.iter() {
            ...
            let js_fun = self.transform_map
                .get(name)
                .ok_or_else(|| Error::SignatureDecryptionError(format!(
...
fn get_transform_plan(js: &str) -> Result<Vec<String>, Error> {
...
fn get_initial_function_name(js: &str) -> Result<&str, Error> {
    lazy_static!{
        static ref FUNCTION_PATTERNS: [Regex; 12] = [
...
fn get_transform_map(js: &str, var: &str) -> Result<HashMap<String, TransformerFn>, Error> {
...
fn map_functions(js_func: &str) -> Result<TransformerFn, Error> {
    lazy_static!{
        static ref MAPPER: [(Regex, TransformerFn); 4] = [
...
fn get_transform_object(js: &str, var: &str) -> Result<String, Error> {
    Ok(
